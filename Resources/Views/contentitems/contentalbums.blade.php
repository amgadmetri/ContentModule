...
<div class="container">
					...
					<div class="caption" align="center">
						<p><h4>{{ $contentAlbum->album_name }}</h4>
							@if(\CMS::permissions()->can('show', 'ContentAlbums'))
								<a class="btn btn-default" href='{{ url("admin/gallery/album/show/$contentAlbum->id") }}' target="_blank">Preview</a>
							@endif
							@if(\CMS::permissions()->can('delete', 'ContentAlbums'))
								<a class="btn btn-default" href='{{ url("admin/content/contentalbums/delete", [$contentItem->id, $contentAlbum->id]) }}' role="button">Delete</a>
							@endif
						</p>
					</div>
				</div>
	...
	@if(\CMS::permissions()->can('add', 'ContentAlbums'))
		<div class="col-sm-2">
